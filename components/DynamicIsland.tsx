...
const DynamicIsland: React.FC<DynamicIslandProps> = ({
  ...
  return (
    <div className="fixed top-4 left-1/2 -translate-x-1/2 z-[100] transition-all duration-500 ease-out animate-in slide-in-from-top-12">
      <div 
        className={`
          relative flex items-center gap-3 md:gap-4 bg-black/90 dark:bg-slate-900/95 backdrop-blur-2xl px-4 py-2 rounded-[2rem] 
          shadow-[0_20px_50px_rgba(0,0,0,0.5)] border border-white/10 dark:border-white/5 transition-all duration-500
          ${isSpeaking ? 'w-[300px] md:w-[350px]' : 'w-[260px] md:w-[310px]'}
        `}
      >
        {/* Visualizer / Icon */}
        ...
        <div className="flex items-center gap-1.5 md:gap-2">
          <button
            ...
            className={`p-2 rounded-full transition-all active:scale-90 border flex items-center justify-center ${
              isMuted 
          ...
          <button
            onClick={onClear}
            className="p-2 rounded-full bg-white/5 border border-white/10 text-white/60 hover:text-white transition-all active:scale-90 flex items-center justify-center"
          >
          ...
          <button
            onClick={onEnd}
            className="bg-red-600 hover:bg-red-500 text-white px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest transition-all active:scale-90"
          >
            End
          </button>
        </div>
